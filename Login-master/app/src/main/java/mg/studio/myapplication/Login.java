...
public class Login extends AppCompatActivity {
    ...
    public void btnLogin(View view) {
        ...
        if (!email.isEmpty() && !password.isEmpty()) {
            ...
            else {// Start fetching the data from the Internet
                //new OnlineCredentialValidation().execute(email,password);
                Toast.makeText(getApplicationContext(),
                        "Internet is unavailable! Please register a new account", Toast.LENGTH_LONG)
                        .show();
                SharedPreferences getinfo = getSharedPreferences("data", MODE_PRIVATE);
                if (getinfo.getString("email", "").equals(email) && getinfo.getString("pwd", "").equals(password)) {
